...
4. **shellenv — post-bootstrap only.** After a bootstrap, `eval "$("$BREW" shellenv)"` runs in-process before the shll hand-off (the freshly installed `shll` lives in the brew prefix and is otherwise command-not-found), and `persist_brew_shellenv` writes the line (e.g. `eval "$(/opt/homebrew/bin/brew shellenv)"`, under a `# Homebrew (added by the shll installer)` comment) into the startup file `brew_rc_file` picks: zsh → `${ZDOTDIR:-$HOME}/.zshenv` on Linux, `${ZDOTDIR:-$HOME}/.zprofile` on macOS (Homebrew's spot — macOS's `/etc/zprofile` path_helper runs after `.zshenv` and would demote brew's bin behind `/usr/bin` in login shells); bash → `~/.bash_profile` on macOS / `~/.bashrc` elsewhere (shll's `resolveRcFile`). The line MUST run before two toolkit blocks: shll's block (its unguarded `eval "$(shll shell-init <shell>)"` needs brew on `PATH`) and run-kit's `# >>> rk tmux guard >>>` block (`brew shellenv` always prepends brew's bin, so running after the guard puts brew's `tmux` ahead of the shim and `rk doctor` FAILs "tmux-guard shim" — the first cut wrote to `.zshrc` and hit exactly this). So it is inserted above the first `# >>> shll >>>` / legacy `# >>> shll shell-init >>>` / `# >>> rk tmux guard >>>` block in that file, otherwise appended (the hand-off's `shll setup shell` and `rk agent setup` then append their blocks after it). For zsh a missing `.zshrc` is created empty, which unblocks `shll setup shell` (it refuses to create rc files). On macOS zsh the rk guard (`.zshenv`) still runs before `.zprofile` — run-kit's documented login-profile limitation, unchanged here. Idempotent (an rc that already mentions `brew shellenv` is untouched), symlink- and mode-preserving (rewrite via `cat >`, never `mv`), and never fatal: an unsupported shell or an unwritable rc falls back to printing the line. Before this, the script only printed the line, and on Linux — where brew's installer also only prints it — every new shell after a fresh bootstrap failed with `command not found: shll`. When brew was already on `PATH`, no bootstrap runs and the rc file is not touched. Covered by `scripts/test-install-rc.sh` and `.github/workflows/installer.yml` (Ubuntu + macOS, including a brew-hidden bootstrap end-to-end).
