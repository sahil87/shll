...
**Placement-directories rule: self-conformant.** The standard's § Placement directories (the two-tier taxonomy — see [cli/standards-content §placement directories](/cli/standards-content.md#placement-directories-the-two-tier-rule)) binds `shll setup agent`'s global placement, and shll conforms: `~/.agents/skills/` is written unconditionally while `~/.claude/skills/` is gated on the `claude` CLI on PATH (`proc.LookPath`), with `--uninstall` and the staleness probe covering both paths regardless (see [cli/setup §the placement set](/cli/setup.md#the-placement-set-two-candidate-paths-cover-four-harnesses-the-claude-write-is-gated)). The placed-skill conformance rules above are unchanged by the tiering. (hb0j)
