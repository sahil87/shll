---
type: memory
description: "shll's conformance state against the 7 toolkit standards: four audited on HEAD against shll v0.0.23 (principles, help-dump, readme-extraction, skill) with two fixes (usage-error exit 2, README badge run) and by-design non-gaps, plus the three producer-surface standards — version behaviorally audited on HEAD and pinned by a conformance test, shell-init conformant by construction (shll the composer), update N/A (shll is the consumer). Includes the conformance-report-in-PR-body convention."
---
