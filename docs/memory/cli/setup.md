...
- **The `description` front-loads two kinds of trigger vocabulary**, both single-sourced from the Roster by `agentSkillDescription()` so they cannot drift from the managed set:
  ...
  - **Extra per-tool sentence(s)** — each non-empty `ProactiveHint` appended verbatim (Roster order) **after** the tool clauses and **before** the closing two-step pointer. This is vocabulary that cannot ride a `SkillHint` clause — capabilities the agent should reach for *unprompted*, plus request-routing whose trigger words would otherwise live only in a tool's own topic index — so the sprawl guard applies: **only run-kit carries a `ProactiveHint` today**, a three-sentence value that does five jobs, written under the ≤1024-byte description cap's compress + relocate rule (triggers stay here; operational prose lives in the body — see the cap bullet below). Sentence one carries the reach-for-unprompted trigger vocabulary for run-kit's agent-proactive capabilities ("to show the user visual content in a browser window, to proxy a local http port to the user's browser, to push a notification to their devices, or to run a VS Code palette command in the user's code editor with `rk code exec`"). Sentence two is a **skill-shadowing counter-instruction** covering two delivery surfaces: "The user may be viewing this session remotely through run-kit's web dashboard — before opening any file or local port in a browser, read `shll skill run-kit`; the same applies before publishing an artifact or hosted page." Sentence three is **tutorial/onboarding routing**: "When the user asks for a tutorial, tour, or onboarding of run-kit or its web dashboard, read `shll skill run-kit tutorial`." — without it, a request like "Onboard me to run-kit" pattern-matches a harness's generic onboarding flow (observed 2026-09-02), because the trigger words lived only inside rk's topic index. Reactive tools stay clause-only; the user's words already name them.

  The trailer names the runtime two-step (`Run 'shll skill' to list … 'shll skill <tool>' for that tool's full usage bundle`). **The description trailer is deliberately NOT extended to the topic form** — it is single-line activation-trigger vocabulary (one YAML line, asserted by `TestAgentSetup_DescriptionSingleLine`), not a teaching surface; the generic topic form belongs in the body's step 2. A *targeted* topic pointer MAY ride a `ProactiveHint` sentence when it is the routing destination (the tutorial sentence's `shll skill run-kit tutorial`) — that is routing vocabulary, not topic-form teaching.
- **The body teaches the runtime discovery steps** (`shll skill` → `shll skill <tool>` → `shll skill <tool> <topic>`) plus a run-kit pointer paragraph carrying the operational prose the description's compress + relocate rule moved out of the frontmatter: the proactive-capabilities line with its examples ("visual display in a browser window (HTML, diagrams, reports, a local dev server), push notifications, and running VS Code palette commands inside the user's code editor (`rk code exec` — refresh a PR list, open a diff, focus a view); see `shll skill run-kit` (and `shll skill run-kit code` for the editor bridge)"), the remote-dashboard explanation ("where `open`/`xdg-open` and localhost URLs never reach them and publishing to a hosted artifact page (e.g. claude.ai) forces them off the dashboard — `shll skill run-kit` carries the proxied-iframe recipe for both cases"), and the tutorial-routing line ("When the user asks for a tutorial, tour, or onboarding of run-kit or its web dashboard, read `shll skill run-kit tutorial` and follow it.") — the body is the right home for operational detail because it loads at activation, exactly when the agent acts on it; it also carries one `shll standards` pointer for toolkit-repo development. Step 2 notes that a large-scope tool's core bundle lists its topic pages and `shll skill <tool> <topic>` serves one on demand (extended by change tp2s). Body text loads only on activation, so the pointer line is activation-cost-only. It only *points at* the runtime steps, so bundles are always fetched from the installed binaries — the placed file stays version-locked in spirit and is refreshed by the installed shll on any re-run (the change-#50 refresh machinery propagates the new body on the next `shll update`).
...
- **≤1024-byte cap (agentskills.io), test-pinned.** The rendered description MUST measure at most `agentSkillDescriptionMaxLen` (1024) bytes — the open spec's `description` cap, which the skill standard's placed-skill conformance rules make binding. `TestAgentSetup_DescriptionWithinAgentSkillsCap` pins it (byte-measured — stricter than a rune count, so a pass holds under any reading of the spec). The current rendering sits at 1023 bytes, so any future trigger-vocabulary addition forces a re-compression: the cap's compress + relocate rule is that trigger coverage outranks prose completeness in the description, and displaced operational prose moves to the skill body. (q8i5)

**`ProactiveHint` holds the complete three-sentence prose verbatim; the builder just appends it** — there is no builder-owned "Also use proactively" preamble composed around a stored fragment. This is the simplest faithful rendering while exactly one tool carries a hint; a composed preamble would be over-engineering. The value does **five** jobs, all load-bearing: **(a) proxy trigger vocabulary** ("to proxy a local http port to the user's browser") matching requests that name proxying/dev servers; **(b) a local-browser skill-shadowing counter-instruction** ("The user may be viewing this session remotely … before opening any file or local port in a browser, read `shll skill run-kit`") that fires the moment any competing skill's local delivery step (`open`/`xdg-open`/localhost URL) is about to run; **(c) a hosted-artifact counter-instruction** ("the same applies before publishing an artifact or hosted page") that fires when an Artifact-style hosted-publishing delivery step — which opens no file and touches no local port — is about to route visuals off the dashboard; **(d) editor-command trigger vocabulary** ("to run a VS Code palette command in the user's code editor with `rk code exec`") matching requests to refresh a PR list, open a diff, or focus a view in the dashboard's `code` lens, where the user never names run-kit; and **(e) tutorial/onboarding routing** ("When the user asks for a tutorial, tour, or onboarding of run-kit or its web dashboard, read `shll skill run-kit tutorial`.") matching requests like "Onboard me to run-kit" that would otherwise pattern-match a harness's generic onboarding flow. All five fire regardless of which skill was activated (see the [skill-shadowing Design Decision](#design-decision-the-proactivehint-does-five-jobs) below). The sprawl guard (only vocabulary that cannot ride a `SkillHint` clause earns description space) is enforced by `TestRosterProactiveHint`, which asserts **exactly run-kit** carries a `ProactiveHint`, that the value appears verbatim in the rendered description, that it is positioned between the tool clauses and the two-step pointer, and — pinning the five functions against a silent rewording — that the rendered description contains the load-bearing fragments `"to proxy a local http port"` (function a), `"before opening any file or local port in a browser, read"` (function b), `"publishing an artifact"` (function c), `"rk code exec"` (function d), and `"tutorial, tour, or onboarding"` + `"shll skill run-kit tutorial"` (function e — triggers and destination). `SkillHint` is unaffected — run-kit's stays `"tmux sessions"` (the reactive task-domain phrase); `TestRosterSkillHints` still enforces the every-tool `SkillHint` contract.
...
### Design Decision: the `ProactiveHint` does five jobs
**Decision**: run-kit's `ProactiveHint` is a three-sentence value doing five load-bearing jobs. Sentence one carries agent-proactive trigger vocabulary (visual display + proxy a local http port + notify + (d) act inside the user's code editor via `rk code exec`, the run-kit code bridge that runs VS Code palette commands from the shell). The code-bridge vocabulary joins sentence one's list rather than forming its own sentence: it is a peer capability, not a counter-instruction, and the single YAML line stays shorter. Sentence two is a counter-instruction with two collision surfaces: (b) the remote-dashboard user, so read `shll skill run-kit` before opening any file or local port in a browser; and (c) the same before publishing an artifact or hosted page — the sentence carries the trigger/collision vocabulary only, with the *why* (local `open`/`xdg-open`/localhost URLs never reach a dashboard viewer; hosted artifact pages force them off the dashboard) and the recipe pointer relocated to the skill body under the ≤1024 cap's compress + relocate rule. Sentence three is (e) request-routing, a distinct job class: it matches the user's *own words* ("tutorial", "tour", "onboarding" of run-kit or its web dashboard) and routes to a specific topic page (`shll skill run-kit tutorial`) — its own sentence (not folded into sentence one's capability list) because it is reactive routing, not a proactive capability; the field's doc comment covers routing vocabulary and the field keeps its name.
...
*Introduced by*: `260721-xv71-runkit-proactivehint-proxy-vocab` (functions a, b); function (c) hosted-artifact counter-instruction by `260722-e09x-runkit-proactivehint-artifact-vocab`; function (d) editor-command vocabulary by `260826-e0gt-code-bridge-skill-vocabulary`; function (e) tutorial/onboarding routing by `260903-qn14-skill-tutorial-onboarding-routing`; compressed under the ≤1024 cap by `260905-q8i5-absorb-agentskills-skill-standard`

### Description capped at 1024 bytes via compress + relocate
**Decision**: The rendered description is bound by `agentSkillDescriptionMaxLen` (1024 — the agentskills.io `description` cap, made binding by the skill standard's placed-skill conformance rules) and test-pinned. To fit, the `ProactiveHint` keeps trigger/collision vocabulary — all six load-bearing fragments `TestRosterProactiveHint` pins survive verbatim — while operational/explanatory prose (capability examples, the remote-dashboard explanation, the proxied-iframe recipe pointer, the palette-command examples) lives in the placed skill's body, which loads at activation.
**Why**: The placed skill is read by every agentskills.io-compatible client; an over-cap description risks truncation or rejection on strict loaders, silently losing exactly the trigger vocabulary it exists to carry. Trigger coverage outranks prose completeness because the description's only job is activation; once activated, the body is in context and carries the operations.
**Rejected**: Softening the cap to a SHOULD in the standard (undermines strict cross-client conformance); deferring the compression to a follow-up change (leaves the publishing repo violating its own standard); rewording the pinned fragments to save bytes (silently weakens the shipped trigger vocabulary the test protects).
