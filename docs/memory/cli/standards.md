...
The roster is **nine** entries, in this order (roster order == output order for both the table and `--json`):
...
3. **Embedded from there** — `//go:embed standards/*.md` into a package-level `var standardsFS embed.FS` in `standards.go` (the same embed bridge `skill.go` uses for the skill bundle — see [cli/skill](/cli/skill.md)).
...
- `TestStandards_UnknownName` — unknown name → `errSilent`, empty stdout, stderr names the offending input and every valid standard (all nine).
- `TestStandardsEmbedMatchesCanonical` — the drift guard: each embedded standard's bytes equal `../../../<SourcePath>` (the canonical `docs/site/standards/` source), failing (naming the file) on drift. Roster-driven, so it covers all nine entries.
