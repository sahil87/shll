...
- **MUST be prompt-free unconditionally** — no interactive prompt in *any* environment, **TTY included**: no confirmation question, no pager, no "press enter to continue", and none surfacing from a wrapped subprocess (a wrapped `brew` call is invoked non-interactively). The clause is deliberately stricter than №1, whose reconciliation blesses `Proceed? [y/N]` on a TTY — `shll update` delegates with **inherited stdio** in the user's own terminal, so stdin typically *is* a TTY mid-compose and a №1-conformant prompt stalls the delegation loop at tool *k* of 6. There is nothing to confirm: an in-place upgrade is not a destructive write in the №5 sense — invoking `update` *is* the consent, and a tool that wants a guard offers `--dry-run`, never a prompt. The clause's **Failure mode** paragraph names the two breakages precisely: on a real TTY with nobody watching (an agent driving `shll update` in a tmux/run-kit pane) the prompt hangs invisibly until the harness times out; when stdin is *not* a TTY the tool refuses fast exactly as №1 requires, and because the delegated argv is fixed (`<tool> update [--skip-brew-update]` — no way to thread `--yes`) the compose hard-fails with no recourse for the caller. A matching `Verifying conformance` bullet requires that no code path reads stdin for a confirmation, in any environment.
...
#### The unconditional prompt-free rule lives on `update.md`, not in principle №1
**Decision**: `update`'s prompt-free obligation is a section of `update.md` that tightens №1 for that one subcommand. №1's own clause text carries no part of the rule; `principles.md` names it only in the contract-table `update` row (Implements: №7, №6, №1) and the companion page summary.
**Why**: №1 protects the headless/agent case and deliberately blesses a TTY-gated `Proceed? [y/N]`, with `shll uninstall` as its reference implementation — right for genuinely destructive commands, wrong for a composed one. Because `shll update` delegates with inherited stdio, a fully №1-conformant `update` stalls the delegation loop while passing every written rule *and* the standard's own conformance checklist, so the loophole closes on the surface where composition makes any prompt unacceptable rather than toolkit-wide. A surface-specific page is where such a tightening belongs — the same shape as this page's brew-handling clause refining №6.
**Rejected**: Tightening №1 toolkit-wide (invalidates the `shll uninstall` reference pattern for commands that genuinely are destructive writes); threading a `--yes`/`--non-interactive` flag through `shll update`'s delegated argv (papers over prompts rather than banning them, and turns a fixed argv into another per-tool capability probe).
*Introduced by*: `260812-igh5-update-standard-prompt-free`.
