...
- **No-range latest fetch failed** — a dedicated `changelog unavailable — {releasesURL}` notice, **not** an `X → X — see {compareURL}` self-compare: when the latest is unknown there is no genuine range to show, so pointing at the releases page is honest; a self-compare would misread as "no change". (r01z)
