...
For a `needsMigration` tool, `upgradeTool` dispatches to `migrateRunKit(ctx, stdout, stderr, t)` instead of delegating. It is **always brew-direct** — never delegate to the old binary (its rename handling is version-dependent and unfixable) and never uninstall→install (that failure-window repair path is the sibling `shll uninstall` change). Steps, **gated on the upgrade succeeding**:
...
4. **Post-migration dual-rack note** (`migrationDualRackNoteFmt`): **re-probe** the legacy formula AFTER the upgrade — if a genuine legacy keg (leaf `rk`) STILL reports installed alongside the now-migrated one, print a one-line cleanup note (`shll uninstall` / `brew uninstall sahil87/tap/rk`). shll never removes the orphan. The dual-rack signal here is DERIVED from the post-migration re-probe, NOT the incoming `probeResult` — a `needsMigration` probe is branch 2, which never sets `dualRack` (the current formula was not-installed pre-migration, so there was no second rack to detect then). (`TestUpdate_MigrationLeftoverDualRackNote`.)

A non-migration run-kit that was classified `dualRack` in branch 1 (state C reached without a migration — the tool is already on the new formula, an orphan `rk` keg lingering) surfaces the same cleanup note from `upgradeTool` directly, after its normal delegated upgrade. (`TestUpdate_MigrationStateC_DualRack`.)
...
## Removal of the 38a6 Linux workaround (change 0854)

The temporary Linux sandbox-trust workaround (backlog `[38a6]`) is **fully removed** by change 0854, which closes its cleanup follow-up `[tkch]`. Previously `shll update`'s brew write calls (`brew update --quiet`, the `brew upgrade sahil87/tap/shll` self-upgrade, and the no-`Update`-argv `brew upgrade <formula>` fallback) carried a `brewEnv()` environment entry that injected `HOMEBREW_NO_REQUIRE_TAP_TRUST=1` on Linux to skip a broken in-sandbox trust re-check in Homebrew 6.0's bubblewrap sandbox.

That bug is **fixed upstream in Homebrew 6.0.4** (the sandbox now exempts the trust file from `deny_read_home`), so the workaround was masking the real (now-mandatory) trust requirement and leaving Linux users silently untrusted. Change 0854:

- Removed `brewEnv()` and the `noRequireTapTrustEnv` constant from `brew.go`, and the `osGoos`-gated injection. (`osGoos` itself stays in `shell_setup.go` for `resolveRcFile`'s darwin-vs-other branch — it is no longer used by any brew call.)
- Reverted all three `update.go` brew write call sites to plain `proc.RunForeground` (no env): the metadata refresh, the shll self-upgrade, and the `upgradeTool` fallback. The `var env []string; if argv[0] == brewBinary { env = brewEnv() }` block in `upgradeTool` is gone.
- Reverted `internal/proc` to its pre-38a6 surface — the `proc.Request.Env` field, `RunForegroundEnv`, and the env-append branch were stripped (no other consumer remained). See [internal/proc](/internal/proc.md).

The correct DX replacing the workaround is the per-formula trust `shll install` now establishes by default — see [cli/install §per-formula trust before install](/cli/install.md#per-formula-trust-before-install-change-0854). `shll update` itself deliberately does **NOT** mutate trust (silently changing trust state on an upgrade command violates least-surprise); it relies on `install` having trusted the tools, and [cli/doctor](/cli/doctor.md#the-trust-sub-check-change-0854) surfaces any installed-but-untrusted tool so the user can re-run `shll install`. The change requires Homebrew ≥ 6.0.4 (documented in the README, not gated in code).

The removed-workaround tests (`TestUpdate_LiveBrewCallsCarryWorkaroundEnvOnLinux`, `TestUpdate_LiveBrewCallsNoWorkaroundEnvOnDarwin`, `TestUpdate_BrewUpgradeFallbackCarriesWorkaroundEnvOnLinux`, `TestUpdate_DelegationNeverCarriesEnvOnLinux`) are gone.
...
This split is a Constitution-aligned choice: probes capture (so shll can branch on the result), user-visible operations foreground (so the user sees brew / the tool working). All foreground writes route through plain `proc.RunForeground` — there is no per-request env override (the 38a6 `proc.RunForegroundEnv` plumbing was reverted in change 0854; see [removal of the 38a6 workaround](#removal-of-the-38a6-linux-workaround-change-0854)).
...
The 38a6 workaround tests (`TestUpdate_LiveBrewCallsCarryWorkaroundEnvOnLinux`/`…NoWorkaroundEnvOnDarwin`, `TestUpdate_BrewUpgradeFallbackCarriesWorkaroundEnvOnLinux`, `TestUpdate_DelegationNeverCarriesEnvOnLinux`) were removed with the workaround (change 0854).
