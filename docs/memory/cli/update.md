...
| `<tool> update [--skip-brew-update]` / `rk desktop update` (delegated upgrade) | `proc.RunStreamedTail` (null stdin, live tee + bounded tail) | User-visible upgrade; the tool's own progress + side-effect output streams to the terminal, and a failure's tail survives the scroll region. |
...
This split is a Constitution-aligned choice: probes capture (so shll can branch on the result), user-visible write operations stream live (so the user sees brew / the tool working) with null stdin (so a prompt attempt fails fast). All write-phase children route through `proc.RunStreamedTail` via the shared `runStreamedChild` helper (`brew.go`) — there is no per-request env override (see [trust posture](#trust-posture-and-the-homebrew-604-floor)). See [Null-stdin streamed children](#null-stdin-streamed-children--the-failure-tail) and [internal/proc §TransportStreamTail](/internal/proc.md#transportstreamtail-used-by-procrunstreamedtail).
...
## Pinned status region (tty two-region write phase)

On a tty, `shll update`'s write phase runs inside a two-region terminal layout (yud0): a pinned one-line status header at the top of the screen and a DECSTBM scroll region beneath it, in which child output streams naturally (no alternate screen, no output withholding). The primitive is `statusRegion` in `src/cmd/shll/region.go`, constructed at write-phase start (after the dry-run/short-circuit returns) with a deferred `stop()` — LIFO-ordered before `progress.remove()`, so the margins restore before the progress state clears.

- **Header content.** `statusHeaderText(verb, tool, pos, total, next, color)` (`region.go`) builds `Updating <tool> (k/n) · next: <tool>` — the honest `k/n` step count over the tools acted on this run (`shll (self)` first when brew-installed, then the installed roster tools in roster order), with a next-tool lookahead omitted on the final step; the run-wide `brew update --quiet` refresh shows the pseudo-step label `brew metadata refresh` (`updateRegionBrewLabel` — a step count would be dishonest there, the refresh is not a roster tool) naming the first actionable tool as `next:`. With color the whole line is one bold-cyan span; the `·` ASCII-degrades to `-` on a non-color run (the run's single `colorEnabled` decision).
- **Gate: tty only, independent of `NO_COLOR`.** Enablement keys on the swappable `regionWriterIsTTY` seam (mirrors `progressWriterIsTTY`) plus a swappable `terminalSize` seam — an enabled region needs a real `*os.File` terminal with usable geometry (≥2 rows, ≥1 col). `NO_COLOR` governs styling only; the region is terminal *state* (the same line OSC 9;4 draws). A disabled instance (pipe, CI, `bytes.Buffer` test writer) is a total no-op — every method writes zero bytes, so non-tty output stays byte-identical in structure (`shll update | cat` is unchanged: no region sequences, no pinned header, no failure-tail block).
- **Restore on every exit path.** Margins reset (`ESC[r`) and the header line is released on normal return and error return via the deferred `stop()`; a SIGINT/SIGTERM handler restores the margins, detaches, and re-raises the signal to the default disposition (no new child-kill logic — children in the foreground process group receive the terminal-generated signal natively; the update standard's brew-safety clause is untouched); SIGWINCH re-queries the size and re-applies margins + header. All mutable state and writer access funnel through one mutex (the watcher goroutine cannot race the command goroutine's header/tee writes). Escape sequences are named constants (`regionMarginSetFmt`, `regionMarginReset`, `regionCursorHome`, `regionCursorPosFmt`, `regionEraseLine`), hand-rolled like `ui.go`'s SGR constants.
- **In-stream headers keep printing.** The `==>`/`▸ [N/M]` per-tool headers still print in region mode — scroll-region content above the last screen never enters scrollback, so the in-stream headers keep tool boundaries greppable and share the degradation path with non-tty runs.

## Null-stdin streamed children + the failure tail

Every write-phase child — `brew update --quiet`, the shll-self `brew upgrade`, the relink heal's `brew link`, each delegated `<tool> update [--skip-brew-update]` / `rk desktop update`, and the brew-upgrade fallbacks — runs through `runStreamedChild(ctx, stdout, stderr, region, name, argv...)` (`src/cmd/shll/brew.go`), the shared helper over `proc.RunStreamedTail` (yud0):
...
- **Bounded failure tail.** The transport captures the last ~4KB of interleaved output (`tailRingSize`); on a child failure (transport error or non-zero exit) in region mode, shll re-prints that tail to stderr framed by `--- last output: <tool> ---` (`failureTailFrameFmt`/`printFailureTail`, `ui.go`), so the cause — including an attempted prompt — stays visible even though lines scrolled out of a DECSTBM region never enter scrollback. Non-tty runs print no tail block (the output is already fully present in the log — the byte-identical guarantee).
...
### Tee-with-pipe transport for region-mode children, not pty and not inherited fds
...
> **Why**: The failure tail requires capture, and DECSTBM-scrolled lines are lost from scrollback, so capture is load-bearing; a tee still streams live (no output withholding). Children seeing a pipe degrade their own tty-only rendering (e.g. brew progress bars) — accepted: the toolkit standard already requires children to behave non-interactively, and degraded child output inside a coherent region beats lost failure context.
> **Rejected**: pty allocation (new dependency, platform surface, overkill for a framing feature); inherited fds with no tail (a mid-run failure's output can scroll away irrecoverably).
...
> **Decision**: The scroll region enables on stdout-tty plus usable geometry alone (the `regionWriterIsTTY`/`terminalSize` seams, mirroring `progressWriterIsTTY`); `NO_COLOR` keeps governing only styling inside the header text.
> **Why**: `NO_COLOR` is a styling convention (no-color.org); a scroll region is terminal state, the same line `progress.go` draws for OSC 9;4. `NO_COLOR` behavior therefore stays exactly as it was.
...
- `TestUpdate_RegionModeSequencesAndTransports` *(yud0)* — forced tty (the `forceRegionTTY` seam helper) → the DECSTBM margin set/reset lifecycle, pinned headers for the brew-refresh pseudo-step and each tool, the in-stream `==>` headers still printing, and every write child (`brew update`, `brew upgrade`, delegated `<tool> update`) carrying `TransportStreamTail` with none left on `TransportForeground`.
- `TestUpdate_RegionShllSelfHeader` *(yud0)* — shll-self is the `[1/2]` pinned-header step with next-tool lookahead naming the first roster tool.
- `TestUpdate_RegionFailurePrintsTail` *(yud0)* — a failing child in region mode re-prints its captured tail to stderr under the `--- last output: <tool> ---` frame.
...
The `progressReporter` itself (sequence forms, gating, NO_COLOR independence, tmux ESC-doubling envelope) is unit-tested in `progress_test.go`, which forces the enabled branch via the `forceProgressTTY` t.Cleanup helper (swapping the `progressWriterIsTTY` seam). The `statusRegion` primitive (byte-exact start/header/stop sequences, resize re-apply, restore-on-stop, disabled no-op, degenerate-geometry disablement, width truncation) and the `statusHeaderText` builder forms are unit-tested in `region_test.go`, which forces the enabled branch via `forceRegionTTY` (swapping the `regionWriterIsTTY`/`terminalSize` seams).
