...
7. **shll self-upgrade (when brew-installed).** If step (4) reported shll itself as brew-installed, run `proc.RunForeground(ctx, brewBinary, "upgrade", shllFormula)` *before* the roster loop. shll has no `update` subcommand to call on itself, so this stays a direct `brew upgrade` (not delegated). See [shll self-upgrade](#shll-self-upgrade) for rationale and edge cases. Failures here go through the same best-effort `anyFailed` path as roster failures.

8. **Sequential per-tool upgrade (delegated).** For each installed tool in roster order, call `upgradeTool(ctx, t, probes[i].supportsSkipFlag)` (`src/cmd/shll/update.go:221`). Dispatch:
   ...
   Best-effort across the roster: on per-tool failure (transport error or non-zero exit), set `anyFailed = true` and `continue` — never abort the loop. After the loop, if `anyFailed`, return `errSilent` (exit 1); else return nil (exit 0).
...
| `<tool> update --help` (capability probe) | `proc.Run` (capture) | Probe — captured so shll can branch on flag support; running concurrently, capturing also avoids output interleaving. |
...
These lock the contract. #2/#3/#9 are reproduced from the original `update` spec; the delegation/probe/parallel-probe decisions come from change cczs's `spec.md`.
