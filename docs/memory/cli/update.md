...
When a prior `shll setup agent` placement exists (`agentSkillPlacementState(env)` — ANY skill target file present under `$HOME`, both candidate paths checked regardless of the claude placement gate), `shll update` ends the write phase by re-running `shll setup agent` as a **subprocess** resolved from PATH (`refreshPlacedAgentSkills` in `agent_setup.go`, header `Refreshing placed agent skills (shll setup agent)…`). A subprocess, not an in-process call, because after a brew self-upgrade the RUNNING binary still holds the OLD embedded skill content — only the freshly installed binary on PATH can place the new bytes. It runs AFTER the roster loop so the run-kit hook delegation inside the refresh uses the just-upgraded run-kit. No placement → no unsolicited writes (silent skip); shll off PATH (dev build) → silent skip; any other failure warns `(continuing)` and never changes the exit code. Because the refresh re-runs the gated install, a Claude Code install that arrives AFTER the initial placement is picked up automatically: the always-present `~/.agents` copy satisfies the placed-gate in the meantime, and the first refresh to run with `claude` on PATH places `~/.claude/skills/` (see [cli/setup §the placement set](/cli/setup.md#the-placement-set-two-candidate-paths-cover-four-harnesses-the-claude-write-is-gated)). Pinned by `TestUpdate_RefreshesPlacedAgentSkills`, `TestUpdate_NoPlacementSkipsRefresh`, `TestUpdate_RefreshFailureWarnsAndContinues`, `TestUpdate_RefreshShllNotOnPathSkipsSilently`.
