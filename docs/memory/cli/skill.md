...
- **`shll skill <tool> <topic>`** → that tool's topic page (`writeSkillTopic`): a Roster tool → `<tool> skill <topic>` passthrough; `shll` self → in-process (reserved `topics` → empty list, exit 0; any other topic → no-topics error, exit 2 — shll ships zero topic pages).
...
### `shll skill shll <topic>` — the self-topic contract, in-process

shll ships **zero** topic pages of its own. `shll skill shll <topic>` is served **in-process, no subprocess** (a self-invocation would recurse into the composer), branching on the topic:

- **`shll skill shll topics`** (the `skill` standard's reserved enumeration topic, `skillReservedTopic`) → the empty list: **zero bytes on stdout, stderr empty, exit 0**. The reserved topic binds every adopting tool — a topic-less tool answers "what topics do you have?" with empty output, keeping the seam uniformly scriptable. Roster tools answer it themselves through the verbatim passthrough (no composer-side special-casing of the reserved name — a tool's conformance is its own obligation).
- **Any other topic** → one stderr line `skillNoTopicsFmt` (`shll skill: shll ships no topic pages (unknown topic %q)`) + `errExitCode{code: usageExitCode}` (exit 2). This matches the sibling unknown-tool usage convention — the standard requires only "non-zero exit, valid topics on stderr", and shll's content-topic set is empty, so naming the unknown topic on one line is the honest diagnostic.

The `skill --help` Long text teaches the reserved topic (`shll skill <tool> topics` lists a tool's topic names, one per line, empty when it ships none) — pinned by `TestSkillCmd_LongHelpNamesReservedTopicsTopic`.
...
All in `skill.go`: `skillEmbedPath` (`"skill/skill.md"`), `skillSubcommand` (`"skill"` — the subcommand shll invokes on each tool, and the default `skillArgv` tail), `skillReservedTopic` (`"topics"` — the standard's reserved enumeration topic, consumed only on the shll-self branch), `skillProbeTimeout` (2s, mirroring `versionTimeout`), `skillHintLine`, `skillUnsupportedFmt` (`"shll skill: %s does not support 'skill' — its installed version may predate it (try 'shll update %s')"` — hedged, takes the tool name **twice**; the `writeSkillBundle` call site passes `tool.Name, tool.Name`), `skillNotInstalledFmt`, `skillNoTopicsFmt` (the `shll skill shll <topic>` no-topics stderr line for non-reserved topics), and `skillTopicTimeoutFmt` (the two-arg deadline/kill exit-1 notice).
