...
## Determinate OSC 9;4 (tty write phase)

On a tty, `shll install`'s write phase emits determinate OSC 9;4 progress and streams its write children through the null-stdin streamed-tail transport (yud0), one linear stream with full scrollback. The shared transport mechanics — null stdin (a prompt attempt reads EOF and fails fast), the live tee, and the bounded tail capture — are documented once under [cli/update §null-stdin streamed children](/cli/update.md#null-stdin-streamed-children), as is the still-waiting heartbeat every write child inherits through `runStreamedChild` ([cli/update §silence heartbeat](/cli/update.md#silence-heartbeat-on-write-phase-children)) (e3x5). Install's own wiring:

- **Construction point.** The `progressReporter` is constructed at write-phase start (`install.go`, after the dry-run branch and the all-already-installed short-circuit return — the dry-run/short-circuit/brew-missing/unknown-target paths construct nothing and emit zero OSC bytes), with `defer progress.remove()`.
- **Streamed-tail children.** Trust (`brewTrustFormula`), each `brew install <formula>`, and each delegated `t.Install` argv (e.g. `rk desktop install`) run through `runStreamedChild` (`brew.go`) — null stdin and a live tee to the run's writers.
...
The design rationale for the shared mechanics lives with update: [tee-with-pipe transport](/cli/update.md#tee-with-pipe-transport-for-write-phase-children-not-pty-and-not-inherited-fds), [stdin hardening scoped to the write-phase children](/cli/update.md#stdin-hardening-scoped-to-the-write-phase-children-only) (install's post-install auto-run steps are in-process seam calls, not subprocesses, so the hardening does not reach them; their own run-kit delegation keeps `proc.RunForeground`).
...
The orchestrator is `runPostInstallSetup(ctx, env, stdout, stderr, color, noShellSetup, noAgentSetup)` (`src/cmd/shll/install.go`). Illustrative happy-path shape on a fresh machine (the second `wrote` line appears only when the `claude` CLI is on PATH — the placement gate; see [cli/setup §the placement set](/cli/setup.md#the-placement-set-two-candidate-paths-cover-four-harnesses-the-claude-write-is-gated)):
...
- `TestInstall_NonTTYFailurePrintsNoTail` *(yud0)* — a failed install child prints no `--- last output ---` tail frame: the child's output is already fully present in the linear stream, and non-tty stdout carries zero escape sequences (the byte-identical guarantee).
...
- **The upstream bootstrap that hands off to this command: [ci/install-bootstrap](/ci/install-bootstrap.md)** (m1zt) — the `curl … | sh` script (`scripts/install.sh`, served at hexokit.com/install), its install-then-update convergence hand-off, and its hexokit.com raw-fetch URL contract.
