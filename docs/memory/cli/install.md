...
- **The upstream bootstrap that hands off to this command: [ci/install-bootstrap](/ci/install-bootstrap.md)** (m1zt) — the `curl … | sh` script (`scripts/install.sh`, served at hexokit.com/install), its install-then-update convergence hand-off, and its shll.ai raw-fetch URL contract.
