---
type: memory
description: "`shll install` — per-formula trust (`--no-trust` opt-out) + `brew install` for missing brew-managed tools; delegated (non-brew) rk-desktop installs via its `Install` argv behind a probe-based platform/prereq gate (skip-with-note, never failing). Post-outcome auto-runs `shll setup shell` + `shll setup agent --yes`. First verb of the `curl … | sh` bootstrap hand-off. tty write phase: pinned DECSTBM status region, determinate OSC 9;4 progress, null-stdin streamed-tail children + failure tail."
---
# cli/install
...
`shll install` is the first verb of the copy-paste install one-liner's hand-off. The bootstrap script `scripts/install.sh` (served at `shll.ai/install`) owns the whole pre-brew phase — it preflights git/CLT, curl, and tmux (consolidated report, per-platform fix commands), bootstraps Homebrew headlessly (`NONINTERACTIVE=1` official installer) when brew is absent and threads the absolute `$BREW` path plus shellenv — then trust-then-installs `shll` itself only if it is missing, and ends install-then-update: `shll install "$@"` (every arg verbatim), then `exec shll update` with the tool names only, so the bootstrap converges the machine to complete and current (already-installed tools are upgraded through their own `update` contracts, e.g. run-kit's daemon restart; freshly installed tools are cheap no-op updates):

```sh
curl -fsSL https://shll.ai/install | sh                # → shll install; then shll update         (whole roster)
curl -fsSL https://shll.ai/install | sh -s -- hop wt   # → shll install hop wt; shll update hop wt (subset)
```

Two implications for `shll install`'s contract:

- **The arg pass-through is part of `shll install`'s public surface.** The [positional tool-name subset args](#positional-tool-name-args--subset-targeting) are what a piped `sh -s -- <tools…>` reaches — and the pass-through to *install* is verbatim, so flags ride it too: `curl -fsSL https://shll.ai/install | sh -s -- --no-agent-setup` lands as `shll install --no-agent-setup`. The bootstrap hands the args straight to `runInstall`, which validates them (`resolveTargets`, `allowShll=false`; unknown/`shll` targets still hard-error, the alias `rk` still resolves to `run-kit`). Only the update pass is filtered: install-only flags are not `shll update` flags, so the script drops dash-prefixed args from the update argv (generic `-*` match — see [ci/install-bootstrap](/ci/install-bootstrap.md)).
- **The script owns the pre-brew phase; `shll install`/`shll update` own everything post-brew.** Preflight, the Homebrew bootstrap, and the shll-self trust/install live in the script; roster knowledge, subset filtering, per-formula trust for the six brew-managed tools, the delegated-tool gate/skip logic, and graceful skips all live here, not in the script (Constitution III). The script's job is the phase `shll install` cannot reach — the user cannot have `shll` without Homebrew having worked. See [ci/install-bootstrap](/ci/install-bootstrap.md) for the script contract and the shll.ai raw-fetch URL contract.
...
5. **Sequential per-tool install, two phases — brew-managed first, then delegated (0854, t26g).** Phase 1 walks `missingBrew`: for each missing brew-managed tool in roster order, print its per-tool header (see [Per-tool output separation](#per-tool-output-separation)), then — when trust is enabled — record per-formula trust via `brewTrustFormula(ctx, stdout, stderr, region, t.Formula)` *immediately before* the action `runChild(t.Name, brewBinary, "install", t.Formula)`. The trust step is interleaved in the per-tool loop (not a separate up-front pass), so trust stays adjacent to the install it unblocks (installed ≠ trusted). Phase 2 walks `missingDelegated` after every brew install — the delegated tools sit behind their runtime prerequisite (rk-desktop behind run-kit), so the prerequisite's brew install has just run when the delegation fires. On a whole-roster run each delegated tool is RE-PROBED at its turn (a failed run-kit install above cascades rk-desktop to a skip-with-note rather than a doomed delegation), then the write delegates to the tool's `Install` argv via the streamed-tail helper — no `brew trust`, no `brew install`. Best-effort across both phases: on per-tool install failure (transport error or non-zero exit), set `anyFailed = true` and `continue`; a delegated skip never sets it. The `[N/M]` counter runs across both phases (`M = len(missingBrew) + len(missingDelegated)`). All write children (trust, `brew install`, the delegated `Install` argv) ride the null-stdin streamed-tail transport via `runStreamedChild` — see [Pinned status region + determinate OSC 9;4](#pinned-status-region--determinate-osc-94-tty-write-phase). See [Per-formula trust before install](#per-formula-trust-before-install) and [The delegated (non-brew) install path](#the-delegated-non-brew-install-path--rk-desktop).
...
- **Stream discipline.** Header and tail go to **stdout** (the stream the install children are streamed onto), never stderr.
...
- **Region mode keeps the in-stream headers.** On a tty the write phase also runs under the pinned status region (see [Pinned status region + determinate OSC 9;4](#pinned-status-region--determinate-osc-94-tty-write-phase)); the `==>`/`▸ [N/M]` headers keep printing inside the scroll region for scrollback continuity — lines scrolled out of a DECSTBM region never enter scrollback.
...
## Pinned status region + determinate OSC 9;4 (tty write phase)

On a tty, `shll install`'s write phase runs the same two-region terminal layout as `shll update` (yud0): the `statusRegion` primitive (`src/cmd/shll/region.go`) pins a one-line status header above a DECSTBM scroll region in which child output streams live. The shared mechanics — the tty-only `regionWriterIsTTY`/`terminalSize` gate (independent of `NO_COLOR`), restore-on-every-exit via deferred `stop()` plus the SIGINT/SIGTERM restore-and-re-raise and SIGWINCH re-apply handlers, the disabled-instance zero-bytes guarantee that keeps non-tty output byte-identical, and the null-stdin streamed-tail transport with its region-mode failure tail — are documented once under [cli/update §pinned status region](/cli/update.md#pinned-status-region-tty-two-region-write-phase) and [§null-stdin streamed children](/cli/update.md#null-stdin-streamed-children--the-failure-tail). Install's own wiring:

- **Construction point.** The region and the `progressReporter` are constructed at write-phase start (`install.go`, after the dry-run branch and the all-already-installed short-circuit return — the dry-run/short-circuit/brew-missing/unknown-target paths construct nothing and emit zero region/OSC bytes), with `defer region.stop()` and `defer progress.remove()` (LIFO: the region restores before the progress state clears, mirroring update).
- **Header text.** `statusHeaderText(installRegionVerb, name, pos, total, next, color)` — `Installing <tool> (k/n) · next: <tool>` (`installRegionVerb = "Installing"`); the lookahead rides `actionableNames`, the flat list across BOTH install phases (missing brew-managed first, then missing delegated), so the `· next:` clause crosses the phase boundary and the final tool omits it.
- **Streamed-tail children.** Trust (`brewTrustFormula`), each `brew install <formula>`, and each delegated `t.Install` argv (e.g. `rk desktop install`) run through `runStreamedChild` (`brew.go`) — null stdin (a prompt attempt reads EOF and fails fast), live tee, and a bounded ~4KB interleaved tail re-printed to stderr under `--- last output: <tool> ---` on failure in region mode only (non-tty runs print no tail block).
- **Determinate OSC 9;4 progress (R7 parity with update).** `newProgressReporter(stderr, env)` — stderr, tmux-passthrough-wrapped, tty-gated, presentation-only — emits `set((pos-1)*100/total)` at each tool boundary inside `installHeader`, an `errorState(pos*100/total)` pulse at each per-tool failure, and a final `set(100)` (all succeeded) or `errorState(100)` (any failure) that holds through the summary tail and post-install setup until the deferred `remove()`. Progress is honest `k/n` steps over the tools acted on — never a synthetic percent.

The design rationale for the shared mechanics lives with update: [tee-with-pipe transport](/cli/update.md#tee-with-pipe-transport-for-region-mode-children-not-pty-and-not-inherited-fds), [stdin hardening scoped to the write-phase children](/cli/update.md#stdin-hardening-scoped-to-the-write-phase-children-only) (install's post-install auto-run steps are in-process seam calls, not subprocesses, so the hardening does not reach them; their own run-kit delegation keeps `proc.RunForeground`), and [the tty-only region gate](/cli/update.md#region-gate-is-tty-only-independent-of-no_color).
...
**Reads run; writes do not.** The probes that partition the roster — `isInstalled` (`brew list --formula --versions`) for brew-managed tools and the delegated `rk desktop status` Probe — still run in dry-run (they are reads, and the preview depends on them) — but **no install write** (no `brew install`, no `rk desktop install`) is performed. The guarantee is structural: the dry-run branch (`install.go:241`) returns before the install loop and before `start := nowFunc()`. `TestInstall_DryRunNoWrites` asserts the `brew list` probe IS recorded, no `brew install <formula>` runs for any tool, and there are **zero `TransportForeground`/`TransportStreamTail` calls** (the two write transports).
...
- `TestInstall_RegionModeSequencesAndTransport` *(yud0)* — forced tty (the `forceRegionTTY` seam helper) → the DECSTBM margin set/reset lifecycle, pinned `Installing <tool> (k/n)` headers with next-tool lookahead across both phases, the in-stream `==>` headers still printing, and every write child (`brew trust`, `brew install`, the delegated `rk desktop install`) carrying `TransportStreamTail` with none left on `TransportForeground`.
- `TestInstall_RegionFailurePrintsTail` / `TestInstall_NonTTYFailurePrintsNoTail` *(yud0)* — a failing child re-prints its captured tail to stderr under the `--- last output: <tool> ---` frame in region mode only; the non-tty failure path prints no tail block (byte-identical guarantee).
- `TestInstall_ProgressEmissionOrder_Success` / `TestInstall_ProgressErrorPulseAndErrorTail` / `TestInstall_ProgressSilentOnNonWritePaths` *(yud0)* — the determinate OSC 9;4 wiring: boundary `set((pos-1)*100/total)`, failure pulses, the final `set(100)`/`errorState(100)` + deferred `remove()`, and zero OSC bytes on the dry-run/short-circuit/brew-missing/unknown-target paths.
...
- **The upstream bootstrap that hands off to this command: [ci/install-bootstrap](/ci/install-bootstrap.md)** (m1zt) — the `curl … | sh` script (`scripts/install.sh`, served at shll.ai/install), its install-then-update convergence hand-off, and its shll.ai raw-fetch URL contract.
