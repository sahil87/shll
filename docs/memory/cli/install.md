---
type: memory
description: "`shll install` — per-formula trust (`--no-trust` opt-out), bootstrap of missing roster tools via `brew install`, idempotent re-run. Post-outcome it auto-runs `shll setup shell` and `shll setup agent --yes` in-process (opt-outs `--no-shell-setup`/`--no-agent-setup`; failures warn + nudge, never failing the install; never under `--dry-run`). The `exec` target of the `curl … | sh` bootstrap: the script owns pre-brew, this command post-brew; arg/flag pass-through is public surface."
---
# cli/install
