...
> **Also reused by `shll install`'s "Next steps" nudge (change 93r2).** `resolveWiringFact` is no longer doctor-only: `shll install` calls it to gate its post-install shell-setup nudge (print the nudge only when `shellResolved && !corrupt && !wired`). The reuse is in-place (same `main` package, no move) and **just as strictly read-only** — `install` never writes the rc file either. The `corrupt`/`shellResolved` edge fields carry the same meaning there (a corrupt block or unresolvable `$SHELL` suppresses the nudge, mirroring doctor's `suggestCorruptBlock`/`suggestShellUnresolvableFmt` distinction). See [cli/install §the post-install "Next steps" nudge](/cli/install.md#the-post-install-next-steps-nudge-change-93r2).
...
- Shared wiring detector: [cli/shell-setup](/cli/shell-setup.md#block-location-and-parsing) — `doctor` reuses `resolveShell`/`resolveRcFile`/`locateBlock`/`blockMatch.hasEval` strictly read-only (never the write paths). Since change 93r2 `doctor`'s `resolveWiringFact` composition is **also** reused (read-only, in place) by [cli/install §the post-install "Next steps" nudge](/cli/install.md#the-post-install-next-steps-nudge-change-93r2) to gate its shell-setup nudge.
