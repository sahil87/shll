...
- A `Long` block (`rootLong`, `src/cmd/shll/root.go`) listing the eight user-facing subcommands and noting that per-tool CLIs continue to work standalone.
...
- `AddCommand` (`src/cmd/shll/root.go`) wires **9 factory funcs**: `newDoctorCmd()`, `newInstallCmd()`, `newUpdateCmd()`, `newChangelogCmd()` (change r01z), `newShellInitCmd()`, `newShellSetupCmd()`, `newVersionCmd()`, `newListCmd()`, and the hidden `newHelpDumpCmd()`. The hidden `help-dump` is not counted in the user-facing surface (see [cli/help-dump-contract](/cli/help-dump-contract.md)), so the **user-facing surface is eight**.

Per Constitution VII (Minimal Surface Area), every top-level subcommand requires explicit justification in the change's intake. The current user-facing surface is eight subcommands (the hidden `help-dump` is not counted — see [cli/help-dump-contract](/cli/help-dump-contract.md)).

### Constitution VII justification per subcommand

These were locked in at spec time (Design Decision #1 for the original three; `install`, `shell-setup` (formerly `shell-install`), `doctor`, and `changelog` were added later, each with its own Constitution VII justification — see [cli/install](/cli/install.md#constitution-vii-justification), the `shell-setup` bullet below, [cli/doctor](/cli/doctor.md), and [cli/changelog](/cli/changelog.md#constitution-vii-justification)) and are reproduced here:
...
| `root.go` | `newRootCmd()` — cobra root that `AddCommand`s 9 factory funcs (8 user-facing — `doctor`, `install`, `update`, `changelog`, `shell-init`, `shell-setup`, `version`, `list` — plus the hidden `help-dump`); also holds the `rootLong` help text listing the eight user-facing subcommands. |
| `tools.go` | `Tool` struct (`Name`, `Formula`, `ShellInit`, `Update`, and `Description`/`Repo` added by change lst7), `Roster`, `formulaPrefix`, `githubOrgBase` (`"https://github.com/sahil87/"`, change lst7 — the repo-URL base for `shll list`), `shellPlaceholder`, `tapName` (`"sahil87/tap"`, the un-suffixed tap name — distinct from `formulaPrefix`'s trailing-slash formula qualifier; added by change l6lo for the then-`brew trust --tap` ceremony, now used only by `doctor`'s tap-level trust check after change 0854 removed `--trust-tap`), the positional-arg target resolver `resolveTargets` + `shllTargetToken` (change b2vg — name-validation only, single-sourced with `Roster`, shared by `update`/`install`; see [update](/cli/update.md#positional-tool-name-args--subset-targeting-change-b2vg)), and the shared shll-self display entry: `shllSelf` (the `Tool`-shaped descriptor), `shllSelfDescription` (`"the manager for the shll toolkit"`), and `shllSelfVersion()` (= `normalizeVersion(version)`), all added by change bb7r — see [the shared `shllSelf` descriptor](#the-shared-shllself-descriptor-change-bb7r). |
| `brew.go` | Shared brew helpers used by the brew-coupled subcommands (`install`, `update`, `doctor`): `hasBrew`, `isInstalled`, `brewBinary`, `brewMissingHint`, `installBrewMissingHint`, `shllFormula`. Also hosts the trust helpers: `brewTrustAvailable` (capability probe via `brew trust --help`), `brewTrustFormula` (`brew trust --formula <formula>` — per-formula trust for `shll install`, change 0854), and `brewTrustList` (`brew trust --json=v1`, JSON-decoded — the read-only trust set for `doctor`'s trust sub-check, change 0854). **Change 0854 removed** the whole-tap `shell-setup --trust-tap` ceremony helpers — `brewTrustTap`, `ensureTapTrust`, `trustHatchHint`, and `brewEnv`/`noRequireTapTrustEnv` (the 38a6 Linux workaround) are gone. `shell-init` and `version` are install-mechanism agnostic and do NOT consult brew — they detect runnable tools via `proc.ErrNotFound` from the sub-tool invocation itself. See [install §per-formula trust](/cli/install.md#per-formula-trust-before-install-change-0854), [doctor §the trust sub-check](/cli/doctor.md#the-trust-sub-check-change-0854), and [update](/cli/update.md). |
| `doctor.go` *(change d0ct; extended by bb7r, 0854)* | `newDoctorCmd()` (registers the `--json` bool flag) + `runDoctor` + `evaluateTool` + `probeVersion` (three-way version state) + `resolveWiringFact` (read-only rc-block check) + `resolveTrustFact`/`trustFact` (read-only `brew trust --json=v1` trust set, change 0854 — the per-installed-tool "formula trusted?" sub-check, WARN on installed-but-untrusted) + the typed `doctorResult` struct (text + JSON render from one source) + marker/suggestion named constants (incl. `suggestNotTrustedFmt`, change 0854). Change bb7r added `shllDoctorResult()` — the always-OK shll-first row built directly (not via `evaluateTool`, no self-subprocess), prepended in `runDoctor` and excluded from the problem-count denominator. Read-only diagnostic. See [doctor](/cli/doctor.md). |
| `install.go` *(extended by bb7r, 0854)* | `newInstallCmd()` + `runInstall` + `shllSelfInstallNote` (`"shll — already present / self-managed"`, change bb7r — the prepended shll-first informational line) + the `--no-trust` flag (`noTrustFlag`/`noTrustFlagUsage`, change 0854 — opts out of the per-formula trust step that now runs by default before each install). See [install](/cli/install.md#per-formula-trust-before-install-change-0854). |
| `update.go` | `newUpdateCmd()` + `runUpdate`; also the change-r01z "What changed:" digest (`versionBump`/`makeBump`, `printUpdateDigest`, `digestSpecs`, digest-literal constants). See [update](/cli/update.md). |
| `changelog.go` *(change r01z)* | `newChangelogCmd()` + `runChangelog` (writer-seam) + the positional `tool@old..new` spec parser (`parseChangelogSpecs`/`splitRange`, `specToolSep`/`specRangeSep` constants) + the concurrent resolver (`resolveChangelog`/`resolveOneSpec`, `resolvedChangelog`/`noteKind`) + the full-body renderer (`renderChangelogResult`, `changelogCapPerTool`). Reuses `resolveTargets` for name validation and [internal/changelog](/internal/changelog.md) for fetch/filter. See [changelog](/cli/changelog.md). |
...
| `version.go` | `newVersionCmd()` + `runVersion`; also the shared install probe `probeToolVersion` + `toolInstalled` (extracted by change lst7, consumed by `version` and `list`). See [version](/cli/version.md). |
...
| `ui.go` *(change y630; extended by 6vuo, r01z)* | Shared UI helper — TTY/`NO_COLOR` detection (`colorEnabled`), the per-tool header printer (`printToolHeader(w, name, pos, total, color)` → `▸ [N/M] <tool>` / `==> [N/M] <tool>`), the summary-tail printer (`printSummaryTail(w, succeeded, total, elapsed, color)` → appends `in <dur>` to both forms), the `formatDuration` helper (`d.Round(time.Second).String()`), the dry-run preview helpers (`previewRow` struct + `printUpdatePreview`/`printInstallPreview` → shared `printPreviewRows` aligned-column printer; header constants `updatePreviewHeaderFmt`/`installPreviewHeaderFmt`; `previewIndent`/`previewGap`), the shell-init comment-separator emitter (`toolComment` → `# ── <tool> ──`), the named ANSI SGR constants (`ansiReset`, `ansiBold`, `ansiBoldCyan`, `ansiGreen`, `ansiRed`), and (change r01z) the changelog-surface glyph-degrade helpers `arrow(color)`/`dash(color)`/`more(color)` with their glyph/ASCII pairs (`→`/`->`, `—`/`--`, `…`/`...`) — shll's OWN Unicode framing degrades to ASCII on a non-TTY/`NO_COLOR` stream per the [per-tool-output-separation spec](/cli/changelog.md#ascii-degrade-per-tool-output-separation-spec) (distinct from the spec-exempted summary-tail em-dash + shell-init box-drawing chars). Holds presentation logic only — **no** subprocess calls, **no** command of its own. Consumed by `update.go`/`install.go` (header + tail + color + preview), `changelog.go`/`update.go`'s digest (glyph helpers), and `shell_init.go` (`toolComment` only — never the color/header path, per the [eval-safety exception](/cli/shell-init.md#the-deliberate-exception--do-not-unify-onto-the--header)). |
