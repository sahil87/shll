...
| `doctor.go` *(change d0ct)* | `newDoctorCmd()` (registers the `--json` bool flag) + `runDoctor` + `evaluateTool` + `probeVersion` (three-way version state) + `resolveWiringFact` (read-only rc-block check) + the typed `doctorResult` struct (text + JSON render from one source) + marker/suggestion named constants. Read-only diagnostic. See [doctor](doctor.md). |
