...
| `tools.go` | `Tool` struct (`Name`, `Formula`, `ShellInit`, `Update`), `Roster`, `formulaPrefix`, `shellPlaceholder`, `tapName` (`"sahil87/tap"`, the `brew trust --tap` argument — distinct from `formulaPrefix`'s trailing-slash formula qualifier; added by change l6lo), and the positional-arg target resolver `resolveTargets` + `shllTargetToken` (change b2vg — name-validation only, single-sourced with `Roster`, shared by `update`/`install`; see [update](update.md#positional-tool-name-args--subset-targeting-change-b2vg)). |
...
- Constitution VII (Minimal Surface Area) → subcommand list is closed at five for v0.1.0 (`install`, `update`, `shell-init`, `shell-setup` (formerly `shell-install`, kept as an alias), `version`); change y630 added behavior to existing commands and a non-command helper file (`ui.go`), not a new subcommand; change ri3h renamed `shell-install` → `shell-setup` (alias preserves the old name) without changing the count; change 6vuo added the `--dry-run` flag to `update`/`install` and the `clock.go` seam — flags and a helper file, not a new subcommand, so the count stays five.
