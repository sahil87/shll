...
### Design Decision: the `ProactiveHint` does two jobs
**Decision**: run-kit's `ProactiveHint` is a two-sentence value: sentence one carries agent-proactive trigger vocabulary (visual display + proxy a local http port + notify), sentence two is a counter-instruction stating that local `open`/`xdg-open`/localhost URLs may never reach a remote-dashboard user and to read `shll skill run-kit` before opening any file or local port in a browser.
**Why**: The placed skill fails to route agents to run-kit's proxy/visual recipe under **skill shadowing** — a competing content-generation plugin (e.g. `visual-explainer:generate-web-diagram`) carries its own complete local-delivery path (`open`/`xdg-open`) and mentions nothing about rk/proxy/iframe, so the harness activates it and the toolkit skill's body never loads. The only shll-owned text guaranteed in every session's context is the placed skill's frontmatter **description** (all installed skills' descriptions are always listed; bodies load only on activation), so the fix lives there. More trigger vocabulary alone would not fire (a request like "show me examples" contains no proxy/remote/port words); the counter-instruction is what collides with the competing skill's delivery step and creates the unresolved gap that sends the agent to `shll skill run-kit`.
**Rejected**: Patching `visual-explainer` (third-party; the shadowing class is open-ended) — changing the skill BODY (loads only on activation, exactly what shadowing prevents) — changing run-kit's own bundle (different repo, already hop-2/unreachable in the failure mode) — a run-kit `agent-setup` session-start hook (the durable, deterministic escalation, deliberately deferred as follow-up). The fix is probabilistic by design — a description line competes with a skill the harness has already committed to and can lose; the run-kit hook is the recorded fallback if it proves insufficient.
*Introduced by*: `260721-xv71-runkit-proactivehint-proxy-vocab`
...
**`ProactiveHint` holds the complete two-sentence prose verbatim; the builder just appends it** — there is no builder-owned "Also use proactively" preamble composed around a stored fragment. This is the simplest faithful rendering while exactly one tool carries a hint; a composed preamble would be over-engineering. The value does **two** jobs, both load-bearing: **(a) proxy trigger vocabulary** ("to proxy a local http port to the user's browser") matching requests that name proxying/dev servers, and **(b) a skill-shadowing counter-instruction** ("The user may be viewing this session remotely … before opening any file or local port in a browser, read `shll skill run-kit`") that fires the moment any competing skill's delivery step (`open`/`xdg-open`/localhost URL) is about to run, regardless of which skill was activated (see the [skill-shadowing Design Decision](#design-decision-the-proactivehint-does-two-jobs) below). The sprawl guard (only agent-proactive capabilities earn description space) is enforced by `TestRosterProactiveHint`, which asserts **exactly run-kit** carries a `ProactiveHint`, that the value appears verbatim in the rendered description, that it is positioned between the tool clauses and the two-step pointer, and — pinning the two functions against a silent rewording — that the rendered description contains the two load-bearing fragments `"to proxy a local http port"` (function a) and `"before opening any file or local port in a browser, read"` (function b). `SkillHint` is unaffected — run-kit's stays `"tmux sessions"` (the reactive task-domain phrase); `TestRosterSkillHints` still enforces the every-tool `SkillHint` contract.
