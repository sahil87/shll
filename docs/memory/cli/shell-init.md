...
Output is concatenated in `Roster` order. This is deterministic (Spec: Composition Order). Today `wt`, `tu`, and `hop` produce output; in the roster (`run-kit, rk-desktop, fab-kit, wt, idea, tu, hop`) the three integrators sit at indices `wt`@3, `tu`@5, `hop`@6, so `runShellInit` emits them in ascending-index order: **`wt` first, then `tu`, then `hop`**. This order is *intentional*, not incidental — it follows the single roster order every surface inherits, so a composed blob reads in the same order `shll list`/`shll version` present the toolkit. `TestShellInit_DeterministicOrder` asserts byte-identical stdout across two consecutive runs. The roster-order invariant itself is guarded by `TestRosterOrder` — see [cli/commands](/cli/commands.md#hardcoded-tool-roster).
...
- Rc-file installer: [cli/setup](/cli/setup.md) (`shll setup shell` — the visible spelling; the hidden `shll shell-setup` and its `shll shell-install` alias dispatch identically) — wraps `shll shell-init <shell>` in an `eval` line and writes it to the user's rc file (idempotent install / `--print` dry-run / `--uninstall` removal).
