...
The centralized subprocess-execution wrapper used by every command in `cmd/shll/`. Constitution Principle I (Security First) requires this routing — **no package outside `internal/proc` may import `os/exec`**. Verified by `cmd/shll/` having zero `os/exec` imports today (acceptance A-029, A-044).
...
```go
// In tests (cmd/shll/update_test.go:33):
...
The fake records every `Request` it receives and returns canned `Result` values (matched by binary name + args). This is how all five `cmd/shll/*_test.go` files avoid spawning real `brew` or per-tool subprocesses.
...
- Routes through this package (verified by no `os/exec` imports outside `internal/proc`).
...
`exitCode(err) (int, bool)` (`proc.go:135`) is the small helper that unwraps `*exec.ExitError` to its `ExitCode()`.
...
- All consumers in `cmd/shll/*.go` — see [cli/commands](../cli/commands.md), [cli/update](../cli/update.md), [cli/shell-init](../cli/shell-init.md), [cli/version](../cli/version.md).
