...
Six companion standards make these principles concrete for every tool repo. Three cover documentation and help: [help-dump](help-dump.md) (the machine-readable help contract), [readme-extraction](readme-extraction.md) (README and `docs/site/` structure), and [skill](skill.md) (the offline, embedded `<tool> skill` agent bundle). Three cover the per-tool surfaces `shll` composes: [update](update.md) (the in-place upgrade contract, with the brew-handling safety rules), [version](version.md) (the `--version` shape shll probes), and [shell-init](shell-init.md) (the eval-safe `shell-init` output shll concatenates).
