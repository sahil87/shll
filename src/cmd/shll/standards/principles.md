...
| [install-composition](install-composition.md) | №7, №8 | binary + repo | No sibling `depends_on` between toolkit formulas; probed runtime siblings with install hints; install docs centralized on hexokit.com |
| [config-home](config-home.md) | №6, №4 | binary | The fixed `$HOME/.config/<tool>/` config root, the `defaults < config file < env < CLI flag` cascade, and the deployment-bootstrap-only env rule |
