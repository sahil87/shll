...
The machine-readable help contract for every CLI in the [HexoKit toolkit](https://hexokit.com/toolkit/). Each tool exposes a hidden `help-dump` subcommand that emits its full command tree as JSON; [hexokit.com](https://hexokit.com) pulls that output on a schedule and renders it as the tool's command reference. Help text is a release artifact, fresh by construction — nothing is hand-copied, and the tool pushes nothing.

This page is the **producer-facing standard**: what your tool must emit. The consumer side — capture, `captured_at` stamping, Zod validation, rendering — is hexokit.com's job, specified with its machine-checkable schema anchor in the [hexokit-site help-dump contract](https://github.com/sahil87/hexokit-site/blob/main/docs/specs/help-dump-contract.md). A tool author's entire obligation is keeping `help-dump` conformant to this page.
...
- **Do not emit `captured_at`.** The capture timestamp is owned by hexokit.com — a tool cannot know its own capture time. The puller stamps it after capture.
