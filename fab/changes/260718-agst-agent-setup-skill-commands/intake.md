...
- **Placement set** (covers all four harnesses with exactly two writes — verified coverage matrix in Origin):
  - `~/.agents/skills/shll-toolkit/SKILL.md` — the agentskills.io open-standard path: read natively by **Codex** (USER scope) and compat-read by **Cursor** and **OpenCode**.
  - `~/.claude/skills/shll-toolkit/SKILL.md` — **Claude Code** (which does not read `~/.agents/`).
  ...
  - Cursor and OpenCode will see the same-name skill from both locations. The bytes are identical, so this is cosmetic at worst; neither harness documents cross-location same-name precedence. Verify at apply; if a harness warns, the fallback is symlinking `~/.claude/skills/shll-toolkit` → the `~/.agents` copy (Claude Code explicitly follows and dedupes symlinked skill dirs).
- **Skill content** — thin, static, portable frontmatter (`name` + `description` ONLY — the OpenCode-recognized common subset; name `shll-toolkit` satisfies the shared `^[a-z0-9]+(-[a-z0-9]+)*$`/match-directory rule). The body teaches the two-step; the description front-loads trigger words (tool names) for implicit activation. Sketch:
