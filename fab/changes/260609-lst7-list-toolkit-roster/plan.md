...
- [x] A-006 R8: `--json` emits a bare JSON array of `{name, description, repo, installed}`, length `len(Roster)`, full repo URL, trailing newline. (writeListJSON list.go:150-171 MarshalIndent("","  ")+"\n"; bare `[]listItem`; repo=repoURL. NOTE: Go's default HTML-escaping renders `&` as `&` in fab-kit's description — valid JSON, jq-decodable, but raw output differs from the table column — see review Should-fix.)
