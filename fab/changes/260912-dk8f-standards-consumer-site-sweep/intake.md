...
> **Update at review-pr (2026-09-12, later the same day)**: X1 merged 16:51Z and X2 merged 16:54Z. Verified live: `https://hexokit.com/shll-ai-redirects.json` → 200; `https://shll.ai/shll/standards/help-dump/` serves a meta-refresh to `https://hexokit.com/shll/standards/help-dump/`; `shll.ai/install` and `shll.ai/versions.json` are byte-identical (md5) to hexokit.com's. The plan's X2 → X4 order is therefore satisfied, and the bootstrap header's "shll.ai/install is a byte copy" is a present fact.
